...
COMMIT;
/*
The issue is: DO $$ ... $$ is treated as an anonymous code block, and it runs in its own transaction context. 
This means you can’t control COMMIT or ROLLBACK inside a DO block.
PostgreSQL doesn't allow you to call COMMIT or ROLLBACK inside a DO block.
The entire DO block is a single transaction — if it runs successfully, it's committed.
If there's an error, the entire transaction is rolled back.
*/
...
-- Trans A
begin;
update tbl_bank_accounts set balance = 500 where account_id = 1;
-- Trans B
begin;
...
Read Committed
->Read Committed only sees data that was committed before the current statement starts.
->Since Transaction A hasn't committed yet, Transaction B sees the old value (450).
-- Trans A
BEGIN;
UPDATE products
SET price = 500
...
Repeatable Read
-> Repeatable Read takes a snapshot of the db when transact begins.
-> All subsequent reads in that transact see the same data, regardless of committed changes by other transaction.
-> so even though Trans B updates and commits the new price 500, Trans A still sees the old value 450 until it ends.
-- Trans A
...
select balance from tbl_bank_accounts where account_id =1; -- still returns old value
commit; -- now updated to the new value as in transaction b
