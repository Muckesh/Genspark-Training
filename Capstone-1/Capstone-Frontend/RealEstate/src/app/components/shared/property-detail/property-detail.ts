...
import { CheckoutModal } from '../../buyer/checkout-modal/checkout-modal';
import { FormsModule, ReactiveFormsModule } from '@angular/forms';

@Component({
  selector: 'app-property-detail',
  imports: [CommonModule, PropertyCard, RouterLink,FormsModule,NgbModule,ReactiveFormsModule],
...
export class PropertyDetail implements OnInit {
  ...
  openPurchaseModal(): void {
    ...
    const modalRef = this.ngModalService.open(CheckoutModal, { centered: true });
    modalRef.componentInstance.property = this.property;
    
    modalRef.result.then((result) => {
      if (result === 'success') {
        this.property.status = 'Sold'; // Update UI immediately
      }
    }).catch(() => {});
  }
